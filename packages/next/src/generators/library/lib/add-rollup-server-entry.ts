 ...
 * second Rollup entry so React Server Components are bundled and exported
 * separately from the client entry.
 */
export function addRollupServerEntry(
  ...
  // Rollup rejects generateExportsField in TS solution setups, where the
  // project package.json owns the exports field.
  const generateExportsField = !options.isUsingTsSolutionConfig;
  ...
  if (tree.exists(rollupConfigPath)) {
    ...
    const updated = content.replace(
      ...
      (line, indent) =>
        line +
        `${indent}additionalEntryPoints: ['./${serverEntry}'],\n` +
        (generateExportsField ? `${indent}generateExportsField: true,\n` : '')
    );
    if (updated === content) {
      throw new Error(
  ...
  if (generateExportsField) {
    build.options.generateExportsField = true;
  }
