...
`
  ...
  if (bundler === 'vite') {
    ...
    addServerExport(host, options, './dist/server');
  } else if (bundler === 'rollup') {
    addRollupServerEntry(host, options);
    if (isTsSolutionSetup) {
      // Rollup generates the exports field at build time in other setups.
      addServerExport(host, options, './dist/server.esm');
    }
...
function addServerExport(
  ...
  updateJson(host, packageJsonPath, (json) => {
    ...
    serverExport.types = `${distEntry}.d.ts`;
    serverExport.import = `${distEntry}.js`;
    serverExport.default = `${distEntry}.js`;
